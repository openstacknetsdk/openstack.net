...
namespace Net.OpenStack.Testing.Integration.Providers.Rackspace
{
    ...
    public class CloudMonitoringTests
    {
        ...
        #region List Audits
        [TestMethod]
        public void Should_List_Audits()
        {
            var provider = new CloudMonitoringProvider();
            var accountInformationResponse = provider.ListAudits(null, _testIdentity);

            Assert.IsNotNull(accountInformationResponse.Values);
            Assert.IsNotNull(accountInformationResponse.Values[1].Id);

            Assert.IsNotNull(accountInformationResponse.Metadata);
            Assert.AreEqual(accountInformationResponse.Metadata.Limit, 100);

        }

        [TestMethod]
        public void Should_Fail_Authentication_Audits()
        {
            var provider = new CloudMonitoringProvider();
            ...
            try
            {
                var accountInformationResponse = provider.ListAudits(null, badIdentity);
            }
            catch (Exception ex)
            {
